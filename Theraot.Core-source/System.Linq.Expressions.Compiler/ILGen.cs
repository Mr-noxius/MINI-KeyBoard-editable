...
internal static class ILGen
{
	...
	{
		System.Dynamic.Utils.ContractUtils.RequiresNotNull(type, "type");
		if (type.IsValueType)
		{
			if (type == typeof(int))
			{
	...
	{
		System.Dynamic.Utils.ContractUtils.RequiresNotNull(fi, "fi");
		if (fi.IsStatic)
		{
	...
	{
		System.Dynamic.Utils.ContractUtils.RequiresNotNull(ci, "ci");
		if (ci.DeclaringType.ContainsGenericParameters)
		{
			throw System.Linq.Expressions.Error.IllegalNewGenericParams(ci.DeclaringType);
		}
	...
	{
		System.Dynamic.Utils.ContractUtils.RequiresNotNull(type, "type");
		System.Dynamic.Utils.ContractUtils.RequiresNotNull(paramTypes, "paramTypes");
		ConstructorInfo constructor = type.GetConstructor(paramTypes);
		if (constructor == null)
		{
			throw System.Linq.Expressions.Error.TypeDoesNotHaveConstructorForTheSignature();
		}
	...
	{
		System.Dynamic.Utils.ContractUtils.RequiresNotNull(value, "value");
	...
	internal static void EmitConvertToType(this ILGenerator il, Type typeFrom, Type typeTo, bool isChecked)
	{
		if (!(typeFrom == typeTo))
		{
			...
			if (typeFrom.IsInterface || typeTo.IsInterface || typeFrom == typeof(object) || typeTo == typeof(object) || typeFrom == typeof(Enum) || typeFrom == typeof(ValueType) || TypeHelper.IsLegalExplicitVariantDelegateConversion(typeFrom, typeTo))
			{
			...
			else if ((!TypeHelper.IsConvertible(typeFrom) || !TypeHelper.IsConvertible(typeTo)) && (nonNullableType.IsAssignableFrom(nonNullableType2) || nonNullableType2.IsAssignableFrom(nonNullableType)))
			{
				il.EmitCastToType(typeFrom, typeTo);
			}
	...
	private static void EmitCastToType(this ILGenerator il, Type typeFrom, Type typeTo)
	{
		...
		else
		{
			if (typeFrom.IsValueType || typeTo.IsValueType)
			{
				throw System.Linq.Expressions.Error.InvalidCast(typeFrom, typeTo);
			}
			il.Emit(OpCodes.Castclass, typeTo);
		}
	}
	...
	{
		bool flag = typeFrom.IsUnsigned();
		bool flag2 = typeFrom.IsFloatingPoint();
		...
		if (isChecked)
		{
			if (flag)
			{
				...
				case TypeCode.UInt64:
					...
					break;
				default:
					throw System.Linq.Expressions.Error.UnhandledConvert(typeTo);
				}
			}
			else
			{
	...
	{
		System.Dynamic.Utils.ContractUtils.RequiresNotNull(items, "items");
	...
	{
		System.Dynamic.Utils.ContractUtils.RequiresNotNull(elementType, "elementType");
		if (emit == null)
		{
			System.Dynamic.Utils.ContractUtils.RequiresNotNull(emit, "emit");
			throw new ArgumentNullException("emit");
		}
		if (count < 0)
		{
			throw System.Linq.Expressions.Error.CountCannotBeNegative();
		}
		il.EmitInt(count);
	...
	{
		System.Dynamic.Utils.ContractUtils.RequiresNotNull(arrayType, "arrayType");
		if (!arrayType.IsArray)
		{
			throw System.Linq.Expressions.Error.ArrayTypeMustBeArray();
		}
