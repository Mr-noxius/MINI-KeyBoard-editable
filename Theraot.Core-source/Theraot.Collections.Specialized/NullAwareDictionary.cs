...
public sealed class NullAwareDictionary<TKey, TValue> : IDictionary<TKey, TValue>, ICollection<KeyValuePair<TKey, TValue>>, IEnumerable<KeyValuePair<TKey, TValue>>, IEnumerable, ISerializable
{
	...
	public IReadOnlyDictionary<TKey, TValue> AsReadOnly => _readOnly;
	...
	public IEnumerator<KeyValuePair<TKey, TValue>> GetEnumerator()
	{
		if (_hasNull)
		{
			yield return new KeyValuePair<TKey, TValue>(_typedNull, _valueForNull[0]);
		}
	...
	{
		if (object.ReferenceEquals(item.Key, null))
		{
			if (_hasNull)
			{
				ClearForNull();
	...
	{
		_hasNull = false;
		_valueForNull = new TValue[1] { default(TValue) };
	}
	...
	private void InitializeNullable()
	{
		...
		_valueForNull = new TValue[1] { default(TValue) };
		_valueComparer = EqualityComparer<TValue>.Default;
		_keys = new ExtendedReadOnlyCollection<TKey>(new EnumerationCollection<TKey>(new ConditionalExtendedEnumerable<TKey>(new TKey[1] { _typedNull }, _dictionary.Keys, () => _hasNull, null)));
		_values = new ExtendedReadOnlyCollection<TValue>(new EnumerationCollection<TValue>(new ConditionalExtendedEnumerable<TValue>(_valueForNull, _dictionary.Values, () => _hasNull, null)));
		_readOnly = new ReadOnlyDictionary<TKey, TValue>(this);
	}
	...
	private void TakeValueForNull(IDictionary<TKey, TValue> dictionary)
	{
		...
		{
			_valueForNull = new TValue[1] { dictionary[_typedNull] };
			_hasNull = true;
		}
		else
		{
