...
public class FormMain : Form
{
	...
	private readonly Color menuBackColor = ModernTheme.Surface;

	private readonly Color menuMouserOverColor = ModernTheme.SurfaceLight;

	private readonly string[] menuStr = new string[5] { "KEY", "Ctrl Shift Alt", "Multimedia", "LED", "Mouse" };
	...
	public FormMain()
	{
		...
		myHidLib.ReportReceived += myHidLib_ReportReceived;
		MenuList();
		KEY_Colour_Init();
		Time_Display_Text();
		Hide_Dowload_Text();
	...
	private void Btn_OnClick(object sender, MouseEventArgs e)
	{
		...
		{
			splitContainer1.Panel2.Controls.Clear();
			LedStudioControl ledStudio = new LedStudioControl();
			ledStudio.Parent = splitContainer1.Panel2;
			ledStudio.Show();
			Key_Clear_Fun();
		...
		ModernTheme.Apply(splitContainer1.Panel2);
	}
		...
		if (WriteMode == 0)
		{
			if (!myHid.Opened)
			{
				...
				if ((int)(myHidPtr = myHid.OpenDevice(vID, pID)) != -1)
				{
					...
					Color backColor = (stateLabel.BackColor = Color.Green);
					label.BackColor = backColor;
				}
				else
				{
					stateLabel.Text = "Not Connected";
					Label label2 = stateLabel;
					Color backColor = (stateLabel.BackColor = Color.Red);
	...
	private void myHidLib_ReportReceived(HidLibrary.HidReport report)
	{
		// Capture only non-empty input reports. This is diagnostic-only and does not write to the keyboard.
		if (report.Data == null || !report.Data.Any(value => value != 0))
		{
			return;
		}
		string line = DateTime.Now.ToString("O") + "  ID=" + report.ReportId + "  " + BitConverter.ToString(report.Data) + Environment.NewLine;
		File.AppendAllText(Path.Combine(Application.StartupPath, "HID-input-capture.log"), line);
	}
	...
	private void Send_WriteFlash_Cmd()
	{
		...
		if (WriteMode == 0)
		{
			...
			if ((byte)myHid.Write(r) == 0)
			{
				...
				Label label = label_Dowload_Dsp;
				Color backColor = (label_Dowload_Dsp.BackColor = Color.Green);
				label.BackColor = backColor;
				Show_Dowload_Text();
			}
			else
			{
				...
				Label label2 = label_Dowload_Dsp;
				Color backColor = (label_Dowload_Dsp.BackColor = Color.Red);
				label2.BackColor = backColor;
				Show_Dowload_Text();
			}
		}
		else
		{
			...
			if (myHidLib.WriteDevice(KeyParam.ReportID, array))
			{
				...
				Label label3 = label_Dowload_Dsp;
				Color backColor = (label_Dowload_Dsp.BackColor = Color.Green);
				label3.BackColor = backColor;
				Show_Dowload_Text();
			}
			else
			{
				...
				Label label4 = label_Dowload_Dsp;
				Color backColor = (label_Dowload_Dsp.BackColor = Color.Red);
				label4.BackColor = backColor;
				Show_Dowload_Text();
			}
	...
	{
		Color keyColour = ModernTheme.SurfaceLight;
		KEY1.BackColor = keyColour;
		KEY2.BackColor = keyColour;
		KEY3.BackColor = keyColour;
		KEY4.BackColor = keyColour;
		KEY5.BackColor = keyColour;
		KEY6.BackColor = keyColour;
		KEY7.BackColor = keyColour;
		KEY8.BackColor = keyColour;
		KEY9.BackColor = keyColour;
		KEY10.BackColor = keyColour;
		KEY11.BackColor = keyColour;
		KEY12.BackColor = keyColour;
		KEY13.BackColor = keyColour;
		KEY14.BackColor = keyColour;
		KEY15.BackColor = keyColour;
		KEY16.BackColor = keyColour;
		K1_Left.BackColor = keyColour;
		K1_Centre.BackColor = keyColour;
		K1_Right.BackColor = keyColour;
		K2_Left.BackColor = keyColour;
		K2_Centre.BackColor = keyColour;
		K2_Right.BackColor = keyColour;
		K3_Left.BackColor = keyColour;
		K3_Centre.BackColor = keyColour;
		K3_Right.BackColor = keyColour;
	}
